    ...
    while(!HandleGraphics::getScenario()->getQuitStatus()){
        ...
        if(HandleGraphics::getScenario()->getSingleStep()){
            if(!gameState->sameState){
                ...
                cout << "passou 0" << endl;
                Mat mapVision = HandleGraphics::getScenario()->getSceneOpenCV();
                if(mapVision.cols == 0 || mapVision.rows == 0)
                    continue;
                ...
                if(strategies.size() > 0){
                    ...
                    strategies[0]->runStrategy(gameState->robotStrategiesTeam,mapVision, mapReward,followPolicy,saveNet);
                    if(strategies.size() == 2){
                        ...
                        strategies[1]->runStrategy(gameState->robotStrategiesAdv,mapVision, mapReward,followPolicy,saveNet);
                    }
                }else{
