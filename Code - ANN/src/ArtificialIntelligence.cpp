...
void ArtificialIntelligence::constructNet() {
    ...
    net << fully_connected_layer<leaky_relu>((SIZE_WIDTH/SCALE_MAP+2)*(SIZE_DEPTH/SCALE_MAP+2), 160)
    << fully_connected_layer<leaky_relu>(160, 160)
    << fully_connected_layer<identity>(160, 4);
...
vec_t ArtificialIntelligence::processImage(){
	...
	for(int i = 0; i < mapVision.size();i++){
		for(int j = 0; j < mapVision.at(0).size();j++){
			...
			switch(index){
				case 0:{
					inputNet.push_back(stdReward);
				}break;
				case 1:{
					inputNet.push_back(0);
				}break;
				case 2:{
					inputNet.push_back(maxReward);
				}break;
				case 5:{
					inputNet.push_back(-stdReward);
				}break;
				case 8:{
					inputNet.push_back(minReward);
				}break;
			}
		}
	}
